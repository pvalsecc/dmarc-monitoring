...
RUA_NAME_FORMAT = re.compile("^(?:[A-Za-z0-9-]+\.[A-Za-z]+[A-Za-z.\-]*!){2}[0-9]+![0-9]+(![a-z0-9-]+)?(?:.xml)?(?:\.zip|\.xml\.gz)$")
...
class ReportDownloader(object):
    ...
    def download(self, destination_folder='./reports'):
        ...
        for num in email_numbers:
            ...
            found, new = parse_email(data[0][1], destination_folder, reports_downloaded)
            if found:
                n_found += 1
            if new:
                n_new += 1
